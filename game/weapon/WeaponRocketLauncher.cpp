...
stateResult_t rvWeaponRocketLauncher::State_Fire ( const stateParms_t& parms ) {
	...
	switch ( parms.stage ) {
		case STAGE_INIT:
			nextAttackTime = gameLocal.time + (240 * owner->PowerUpModifier ( PMOD_FIRERATE ));		
			Attack ( false, 10, 18, 0, 0.3f );
	...
	switch ( parms.stage ) {
		case STAGE_INIT: {
			...
			if ( animNum ) {
				idAnim* anim;
				anim = (idAnim*)viewModel->GetAnimator()->GetAnim ( animNum );				
				anim->SetPlaybackRate ( (float)anim->Length() / (240 * owner->PowerUpModifier ( PMOD_FIRERATE )) );
			}
